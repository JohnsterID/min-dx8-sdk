...
int test_device_creation_windowed(void) {
    ...
    /* Create device - try Hardware VP first, then Software VP */
    printf("\n[4] Creating D3D device (HAL)...\n");

    /* Try Hardware Vertex Processing first */
    printf("  [4a] Attempting Hardware Vertex Processing...\n");
    ...
    if (SUCCEEDED(hr)) {
        printf("[OK] HAL device created with Hardware VP\n");
    } else {
        printf("  [INFO] Hardware VP failed (hr=0x%08lX), trying Software VP...\n", hr);

        /* Fallback to Software Vertex Processing */
        printf("  [4b] Attempting Software Vertex Processing...\n");
        hr = IDirect3D8_CreateDevice(
            pD3D,
            D3DADAPTER_DEFAULT,
            D3DDEVTYPE_HAL,
        ...
        if (SUCCEEDED(hr)) {
            printf("[OK] HAL device created with Software VP\n");
        } else {
            printf("[FAIL] Both Hardware and Software VP failed: hr=0x%08lX\n", hr);
            ...
            /* Try reference rasterizer as last resort */
            hr = IDirect3D8_CreateDevice(
                pD3D,
                D3DADAPTER_DEFAULT,
                D3DDEVTYPE_REF,
